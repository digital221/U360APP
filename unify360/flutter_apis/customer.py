			...
			if data:
				...
				if data.get("name"):
					...
					if c.name and data.get("filename") and data.get("file_data"):
						...
						if f_.name:
							frappe.db.set_value("Customer", c.name, "image", f_.file_url)
					frappe.db.commit()
					if c.name:
						data_to_send = {"customer_id": c.name}
						make_response(success=True, message="Customer Created.", data=data_to_send)
					else:
						make_response(success=False, message="Customer Name Not Found.")
				else:
					make_response(success=False, message="Failed to Create Customer, Please Try Again!")
			else:
		...
		if user_details:
			...
			if data:
				...
				if check:
					...
					if data.get("name") or data.get("mobile_no") or data.get("location") or data.get("territory") or data.get("email_id"):
					...
					make_response(success=False, message="Customer not found against this Id !")
			else:
				make_response(success=False, message="Data not Found!")
		else:
			make_response(success=False, message="Invalid user!")
...
def get_customers(limit=None):
	try:
		...
		if user_details:
			...
			fields=["name as id",  "customer_name as name", "image", "custom_location", "territory", "email_id","mobile_no",],
			order_by='creation desc', page_length=cint(limit) if limit else 0)
